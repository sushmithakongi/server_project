...
export const ListRequestforuser = () => {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            <button
              ...
              onChange={() => console.log(value, tableMeta.rowData[0])}
              onClick={() => {
                infcmt(tableMeta.rowData[0]);
                setIndes(tableMeta?.rowIndex + 1);
              }}
            >
              <MarkUnreadChatAltRoundedIcon />
            </button>
  ...
  async function submitcomment(e) {
    ...
    const past = await api.post("user_chat", raw);
    let d = "";
    console.log(past?.data?.User_chat[0]?.Comment, "<----past");
    const data = past?.data?.User_chat[0]?.Comment?.map((itm) => {
      const date = `${itm[0] ? itm[0] : ""},${itm[1] ? itm[1] : ""}`;
      return date;
      console.log(itm, "<------dead");
    });
    console.log(data?.toString(), "<----yutyutyutuytyutyu", d);
    let a = {
      Id: indes,
      User_Comments: data?.toString() + "," + User_Comments,
    };
  ...
  const infraChat = async (id) => {
    ...
    await api.post("infra_chat", JSON?.stringify(data)).then((res) => {
      ...
      setDataUser(res.data.Infra_chat);
      let arr = res.data.Infra_chat;
      console.log(arr, "arr");

      console.log(typeof arr, "arr");
      arr.map((ans) => {
        console.log("ingggggg", ans.Comment);
        if (ans.Id === ID) {
          if (ans.Comment === []) {
            setDoop(["no msg "]);
        ...
        console.log("testing nnn", ans.Comment);
        // return (doop = ans.Comment);
      });
      // console.log("doop",a);
                ...
                <Card sx={{ minWidth: 300 }}>
                  ...
                  <CardContent>
                    {dataUser?.map((items, i) => {
                      console.log(items, "<----gettingggg");
                      return (
                        <div>
                          {items?.Comment?.map((subitems, o) => {
                            {
